...
| **GIST-004** | Specification mis-typed as Category | OntOLinter | A class is `rdfs:subClassOf gist:Category` and its local name matches a pattern (`Status`, `State`, `Level`, `Score`, `Capability`, `Readiness`, `Configuration`, `Condition`) that typically denotes a time-bounded fact rather than a type enumeration — for example `AMLAlertStatus` or `CustomerOnboardingStatus`. This is a heuristic: the rule raises a question for the modeller, not a verdict. The right fix may be re-typing as `gist:Specification`, switching to `gist:Magnitude`, removing the class, or keeping the Category typing if the name is just misleading. |
